...
if [ "${SETUP}" = 1 ]; then
  ...
  if [ "${OS}" = "Linux" ]; then
    ...
    sudo apt-get update
    sudo apt-get -y install tree cmake git build-essential qtbase5-dev qt5-qmake libmagick++-dev ffmpeg poppler-utils
...
if [ "${OS}" = "Linux" ]; then
    mkdir $CWD/build1
    cd $CWD/build1
    qmake -qt=qt5 CONFIG+=release PREFIX=/usr CONFIG+=with_magick CONFIG+=oldmagick ..
    make
